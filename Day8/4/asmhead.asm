...
VRAM    EQU     0x0ff8      ; VIDEO RAM  | グラフィックバッファの開始番地
        ...
        ; こいつをCLI(割り込み禁止)前にやっておかないと、たまにハングアップする
        ; PICの初期化はあとでやる

        MOV     AL, 0xff		;io_out(PIC0_IWR,0xff)/*マスタの全部のPICの割り込み禁止*/
        OUT     0x21, AL		;io_out8(PIC1_IWR,0xff)/**スレーブの全部のPICの割り込み禁止/
        NOP                   ; OUT命令を連続させるとうまくいかない機種があるらしいので
        OUT     0xa1, AL

        CLI                   ; さらにCPUレベルでも割り込み禁止(io_cli())
...
;		**/
        ...
        ; プロテクトモード移行
        ; プロテクトモードはGDTを用いる(仮想メモリを用いる)
        ;[INSTRSET "i486p"]    ; i486の命令まで使いたいという記述
        ; ここで指定するのではなくgccでcompileする際にi486で指定

        LGDT [GDTR0]   ; 暫定GDTを設定 (GDTの場所を格納するGDTRのデータをセットするための命令)
        MOV  EAX,CR0
        AND  EAX,0x7fffffff ; bit31を0にする（ページング禁止のため）
        OR  EAX,0x00000001 ; bit0を1にする（プロテクトモード移行のため）
        MOV  CR0,EAX
        JMP  pipelineflush	;ここでjmpするのは16から32bitにして機械語の解釈を変更するため(実行中に解釈を解釈を開始するパイプライン処理を実行している)
pipelineflush:
        MOV  AX,1*8   ;  読み書き可能セグメント32bit (0x00000008はセグメントレジスタの値を変更している。これはgdt+1に該当する)
        MOV  DS,AX
        MOV  ES,AX
        MOV  FS,AX
        MOV  GS,AX		
