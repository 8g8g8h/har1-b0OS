...
void console_task(struct SHEET *sheet,int memtotal){
  ...
  cons.cur_y=28;
  cons.cur_c=-1;
  task->cons=&cons; /*ここでconsのアドレスを別のところに書くようにしている*/ 
...
void cons_runcmd(char *cmdline,struct CONSOLE *cons,int *fat,int memtotal){
  ...
  }else if(ostrcmp(cmdline,"exit")==0){
    cmd_exit(cons,fat);
  }else if(cmdline[0]!=0){
...
void cmd_exit(struct CONSOLE *cons,int *fat){
  ...
  _io_cli();
  fifo32_put(fifo,cons->sht-shtctl->sheets0+768);//ここで、コンソールタスクからtask_aにデータを送って終了するというデータを送る
  _io_sti();
  for(;;){
    task_sleep(task);//このタスクは終了をtask_aにお願いしているので、ずっとその間は、スリープしておく
  }
}
int cmd_app(struct CONSOLE *cons,int *fat,char *cmdline){
  ...
  if(finfo!=0){
    ...
    if(finfo->size>=36&&ostrncmp(p+4,"Hari",4)==0&&*p==0x00){
      ...
      task->ds_base=(int)q;
      set_segmdesc(gdt+task->sel/8+1000,finfo->size-1,(int)p,AR_CODE32_ER+0x60);/*アプリのコードセグメントを設定してあげている*/
      set_segmdesc(gdt+task->sel/8+2000,segsiz-1,(int)q,AR_DATA32_RW+0x60);/*アプリのデータセグメントを設定してあげている*/
      for(i=0;i<datsiz;i++){
        q[esp+i]=p[dathrb+i];/*ここで、hrbファイル内のデータをデータセグメントにコピーしている*/
      }
      start_app(0x1b,task->sel+1000*8,esp,task->sel+2000*8,&(task->tss.esp0));/*アプリを展開する際にアプリを実行することをタスクとしてカウントしている。つまりタスクとしてアプリを実行している*/
      shtctl=(struct SHTCTL *)*((int *)0x0fe4);
      for(i=0;i<MAX_SHEETS;i++){
        sht=&(shtctl->sheets0[i]);
        if((sht->flags&0x11)==0x11&&sht->task==task){
          /*アプリが開きっぱなしにした下敷きを発見*/
          sheet_free(sht);/*閉じている*/
        }
      }
...
int *hrb_api(int edi,int esi,int ebp,int esp,int ebx,int edx,int ecx,int eax){
  ...
  if(edx==1){
    ...
    sht->task=task;
    sht->flags|=0x10;/*自動クローズが有効になるようにしている*/
    sheet_setbuf(sht,(char *)ebx+ds_base,esi,edi,eax);
    make_window8((char *)ebx+ds_base,esi,edi,(char *)ecx+ds_base,0);
    sheet_slide(sht,((shtctl->xsize-esi)/2)&~3,(shtctl->ysize-edi)/2);
    sheet_updown(sht,shtctl->top);/*高さを一番上に設定*/
    regs[7]=(int)sht;
  }else if(edx==6){
    ...
    memman_init((struct MEMMAN *)(ebx+ds_base)); /*memman_initでメモリについて初期化(ちなみに、structでmemman の構造にしないといけない)*/
    ...
    for(;;){
      ...
      if(fifo32_status(&task->fifo)==0){
        if(eax!=0){
          task_sleep(task);/*FIFOの中身がないので、そのまま何もしないでスリープ*/
        }else{
          _io_sti();
          regs[7]=-1;//ここでアプリを強制的に終了している
          return 0;
        }
      }
      i=fifo32_get(&task->fifo);
      ...
      if(i>=256){
		...
		regs[7] = (int) timer_alloc();
    ((struct TIMER *)regs[7])->flags2=1;//自動キャンセル有効にする
  } else if (edx == 17) {
...
void hrb_api_linewin(struct SHEET * sht,int x0,int y0,int x1,int y1,int col){
  ...
  dx=x1-x0;/*横方向にどれだけ動くか*/
  dy=y1-y0;
  x=x0<<10;/*ここで、xを1024倍している*/
  y=y0<<10;
  if(dx<0){/*ここから下は、万が一最初の方向がマイナスに出た場合でもあとで修正するためにここでは、一回プラスにしている。*/
    dx=-dx;
  }
  if(dy<0){
    dy=-dy;
  ...
  if(dx>=dy){
    len=dx+1;/*先に長さを決める(ここで、dxのほうが長いのでこれを基準にすれば長さが短くなることはない。(if文のこと))*/
    if(x0>x1){
      dx=-1024;//ここでマイナス方向に移動するようにしている(-1の変化)
    }else{
      dx=1024;
    }
    if(y0<=y1){
