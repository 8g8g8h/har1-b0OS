...
void HariMain(void){
    ...
    for(i=0;i<3;i++){
      ...
      //tss_bの初期値設定
      task_b[i]->tss.eip=(int) &task_b_main;//eipは、このタスクに変わっ たときの実行してほしいアドレスを示してる。
      task_b[i]->tss.es=1*8;
      task_b[i]->tss.cs=2*8;
      task_b[i]->tss.ss=1*8;
      task_b[i]->tss.ds=1*8;
      ...
      *((int *)(task_b[i]->tss.esp+4))=(int)sht_win_b[i];
      //タスクを動かすためにrunningなどを増やし、使っているリストに追加している
      //実際にまだ動いてはいない
      task_run(task_b[i],2,i+1);
      //task_bが動いているのは、タイマで0.02秒ずつ行われている。
      //これは、タイマの方で設定している。
    }    
    ...
    sheet_slide(sht_win_b[0],200,400);
    sheet_slide(sht_win_b[1],300,500);
    sheet_slide(sht_win_b[2],400,600);
