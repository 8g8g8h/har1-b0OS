...
void task_run(struct TASK *task,int level,int priority){
  ...
  if(task->flags!=2){
    /*スリープから起こされる場合。または、上のif文にある、動作レベルを下げたり上げたりした場合にその希望するレベルに今回実行したいタスクを追加してあげることが必要*/
    task->level=level;
    task_add(task);
  }
...
void task_switch(void){
  ...
  tl->now++;//ここで該当タスクを横にずらしてあげている。よってタスクが２つ以上あった場合はnow_taskと下で出てくるnew_taskが異なるものを示すためにタスク切り替えが行われる。
  if(tl->now==tl->running){
    tl->now=0;
  }
  //task_runでtaskctlのどれかのレベルにあるタスクを追加したために、lv_changeでタスクのレベルを調べる必要がある。一番上のもののタスクを実行しないといけない。
  if(taskctl->lv_change!=0){
    task_switchsub();
    tl=&taskctl->level[taskctl->now_lv];//ここで一番上のレベルをtlに代入して示してあげている。
  }
...
void task_sleep(struct TASK *task){
  ...
  if(task->flags==2){
    ...
    if(task==now_task){
      /*自分自身のスリープであるために、自分をスリープして何もしなかったら動かなくなるので、他のタスクにタスクスイッチする必要がある*/
      task_switchsub();//ここでは、次にどのレベルを動かすかを決めている。この関数では、一番上を調べるようにしている。
      now_task=task_now();//一番上のレベルの「現在のタスク」を教えてもらう
