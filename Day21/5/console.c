...
int cmd_app(struct CONSOLE *cons,int *fat,char *cmdline){
  ...
  if(finfo!=0){
    ...
    *((int *)0xfe8)=(int)p;//ここでアプリを展開するメモリがわかるのでこれにより、msgの内容を読むことができる。(CSでメモリの位置を指定していた(1文字表示の場合)ことをやらずに直接どこのメモリにこのアプリが読み込まれるかがわかるためにアドレスメッセージを読むことができる)
    file_loadfile(finfo->clustno,finfo->size,p,fat,(char *)(ADR_DISKIMG+0x003e00));
    set_segmdesc(gdt+1003,finfo->size-1,(int)p,AR_CODE32_ER);/*アプリのコードセグメントを設定してあげている*/
    set_segmdesc(gdt+1004,64*1024-1,(int)q,AR_DATA32_RW);/*アプリのデータセグメントを設定してあげている*/
    if(finfo->size>=8&&ostrncmp(p+4,"Hari",4)==0){
      p[0]=0xe8;
      p[1]=0x16;
      p[2]=0x00;
      p[3]=0x00;
    ...
    start_app(0,1003*8,64*1024,1004*8);/*アプリを起動するための関数far-callの他に、ESPなどの設定も行っている*/
    memman_free_4k(memman,(int)p,finfo->size);
