  ...
  if(finfo!=0){
    ...
    if(finfo->size>=36&&ostrncmp(p+4,"Hari",4)==0&&*p==0x00){
      ...
      *((int *)0xfe8)=(int)q;
      ...
      for(i=0;i<datsiz;i++){
        q[esp+i]=p[dathrb+i];/*ここで、hrbファイル内のデータをデータセグメントにコピーしている(ここで、espの後にしっかりとデータ部分を置くことが可能となる)*/
      }
      start_app(0x1b,1003*8,esp,1004*8,&(task->tss.esp0));//ここで、0x1bからスタートして、espを指定することにより、しっかりデータを扱える
