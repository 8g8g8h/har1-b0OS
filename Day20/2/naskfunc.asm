...
_asm_cons_putchar:  ;void cons_char(CONSOLE *cons,int chr,char move )
    PUSH  1         ;char move
    AND   EAX,0xff ;AHやEAXの上位を0にして、EAXに文字コードが入った状態にする
    PUSH  EAX       ;int chr
    PUSH  DWORD [0x0fec]  ;一旦ここにconsのアドレスをメモする これが必要なのは、cons_putcharの引数がconsであるが、これはOS側しかわからないので、OS側がメモリに書いてあげる必要がある CONSOLE *cons
    CALL  cons_putchar    ;この段階で裏でスタックに戻ってくるためにアドレスが積まれている。ちなみにここは、externでcons_putcharのアドレスを取得しているためにできる
