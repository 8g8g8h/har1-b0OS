...
farcall: ;void farcall(int eip,int cs); これは、farjmpと同じようなcallである。これにより、retで関数に返ってくることができる
    CALL  FAR [ESP+4]
    RET
...
_asm_cons_putchar:
    ...
    PUSH  EAX
    PUSH  DWORD [0x0fec]  ;一旦ここにconsのアドレスをメモする これが必要なのは、cons_putcharの引数がconsであるが、これはOS側しかわからないので、OS側がメモリに書いてあげる必要がある
    CALL  cons_putchar
