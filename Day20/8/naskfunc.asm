...
asm_hrb_api:
    STI  
    PUSHAD  ;これは、INT0x40をされたときのECXなどを保存するためのPUSH(ここでスタックに積んでいるレジスタの順番は、eax ecx edx ebx esp ebp esi edi)。これをやらないとINT0x40をやった後のECXが変わってしまい、適当な場所を示してしまうために必ず最初に該当するECXを保存しておく必要がある
    PUSHAD  ;hrb_apiに渡すためのPUSH(これは、hrb_apiに引数が必要であるためにここで再び渡すためのPUSHADを行っている。順番は先程と一緒)
    CALL  hrb_api
