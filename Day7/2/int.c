    ...
    /*PS/2キーボードからの割り込み*/
{
    ...
    unsigned char data;
    _io_out8(PIC0_OCW2,0x61); /*IRQ-01受付完了をPICに通知(ここでデータがあるかもということをPICに知らせる)*/
    data = _io_in8(PORT_KEYDAT); /*ここで実際のデータを送る*/
	//ここでバッファにデータが入ってないことを確認
    if(keybuf.flag==0){
        keybuf.data=data;
        keybuf.flag=1;
    }
...
{
    struct BOOTINFO *binfo=(struct BOOTINFO *) ADR_BOOTINFO;
    bookfill8(binfo->vram,binfo->scrnx,COL8_000000,0,0,32*8-1,15);
