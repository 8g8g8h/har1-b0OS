...
; 画面モードをメモする
        ...
        MOV     DWORD [VRAM], 0x000a0000  ; Video RAM
                                          ...
                                          ;
                                          ; > [VRAM]に 0xa0000 を入れるのですが、PC の世界で VRAM はビデオラムのことで
                                          ; > 「video RAM」と書き、画面用のメモリのこと。このメモリは、もちろんデータを記憶することがい
                                          ; > つも通りできる。しかしVRAMは普通のメモリ以上の存在で、それぞれの番地が画面上の画素に対応
                                          ; > していて、これを利用することで画面に絵を出すことができる。
