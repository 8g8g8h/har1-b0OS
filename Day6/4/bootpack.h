...
struct SEGMENT_DESCRIPTOR{
	...
	//baseとはセグメントの番地(32bit)でありこれを分けて書いただけ
	//limitはセグメントの大きさ(20bit)しか用意していない
	//しかしこれでは1MBしか表現できない→　ここでGbit(これが1だとページ単位であると解釈できる)
	//ページ単位とはcpuでのページの単位(1ページとは4kb)(振り分けはhighの上位4bit以下+low)
	//accsess_rightの4bitは拡張アクセス権であり、下位8bitは以下のようになっている
	//これは126Pはセグメント属性に書いてあるので参照
